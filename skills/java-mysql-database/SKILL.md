...
description: 阿里巴巴Java开发手册MySQL数据库规约，包含建表规约、索引规约、SQL语句、ORM映射。Use when designing database schemas, writing SQL queries, or working with MySQL in Java applications. Make sure to use this skill whenever the user mentions database design, table schema, column naming, index design, SQL optimization, query performance, ORM mapping, MyBatis, Hibernate, result mapping, decimal types, varchar limits, logical deletion, or any MySQL/database best practices in Java projects, even if they don't explicitly mention 'Alibaba' or 'database standards'.
---
