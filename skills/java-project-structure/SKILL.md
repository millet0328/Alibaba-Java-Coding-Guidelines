...
description: 阿里巴巴Java开发手册工程结构规约，包含应用分层、二方库依赖、服务器。Use when structuring Java projects, managing dependencies, or organizing project architecture. Make sure to use this skill whenever the user asks about project structure, application layering, MVC architecture, Service/DAO/Manager layers, Maven dependencies, pom.xml configuration, GAV coordinates, version management, SNAPSHOT vs RELEASE, jar conflicts, server configuration, timeout settings, JVM parameters, TCP settings, or Java project architecture best practices, even if they don't explicitly mention 'Alibaba' or 'project structure standards'.
---
