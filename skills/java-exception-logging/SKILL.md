...
description: 阿里巴巴Java开发手册异常日志规约，包含错误码、异常处理、日志规约。Use when handling exceptions, logging, or error codes in Java applications. Make sure to use this skill whenever the user mentions error handling, exception catching, try-catch, logging configuration, log levels, error codes, SLF4J, Logback, Log4j, exception design, NPE prevention, or log best practices in Java, even if they don't explicitly mention 'Alibaba' or 'error handling standards'.
---
